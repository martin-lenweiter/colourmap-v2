...
/* ═══════════════════════════════════════════════════════════
   STAR COMPASS — Sharing ring compass
   S(left) T(top) A(right) R(bottom)
   Includes: STAR blobs, Distant/Connected columns, sub-cells, 3-step programs
   ═══════════════════════════════════════════════════════════ */
...
const STORAGE_KEY = 'colourmap:share-values';
const BLOCKED_KEY = 'colourmap:share-distant';
const MOVING_KEY = 'colourmap:share-moving';

function loadValues(): Record<ShareAxis, number> {
  try {
  ...
  return { share: 50, authentic: 50, roots: 50, express: 50 };
}
...
export default function ShareCompass() {
  ...
  const [distantItems, setDistantItems] = useState<string[]>(() => loadList(BLOCKED_KEY));
  const [movingItems, setConnectedItems] = useState<string[]>(() => loadList(MOVING_KEY));
  const [distantInput, setDistantInput] = useState('');
  const [movingInput, setConnectedInput] = useState('');

  const handleRating = (key: ShareAxis, value: number) => {
    const next = { ...values, [key]: value };
    setValues(next);
    localStorage.setItem(STORAGE_KEY, JSON.stringify(next));
  };

  const addDistant = (text: string) => {
    if (!text.trim()) return;
    const next = [...distantItems, text.trim()];
    setDistantItems(next);
    localStorage.setItem(BLOCKED_KEY, JSON.stringify(next));
    setDistantInput('');
  };
  const addConnected = (text: string) => {
    if (!text.trim()) return;
    const next = [...movingItems, text.trim()];
    setConnectedItems(next);
    localStorage.setItem(MOVING_KEY, JSON.stringify(next));
    setConnectedInput('');
  };
  const removeDistant = (i: number) => {
    const next = distantItems.filter((_, idx) => idx !== i);
    setDistantItems(next);
    localStorage.setItem(BLOCKED_KEY, JSON.stringify(next));
  };
  const removeConnected = (i: number) => {
    ...
    >
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2 text-center">
          ...
          {distantItems.length > 0 && (
            <div className="flex flex-wrap justify-center gap-1">
              {distantItems.map((item, i) => (
                <span
                  key={i}
                  onClick={() => removeDistant(i)}
          ...
          <input
            type="text"
            value={distantInput}
            onChange={(e) => setDistantInput(e.target.value)}
            onKeyDown={(e) => {
