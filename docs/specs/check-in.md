...
- The Emotions lane may use one atmospheric backdrop image at a time, rotating slowly between compressed mood images such as night city and sunset scenes. These images are mood support only: they must stay subtle, compressed, and covered by overlays so all text remains readable. The image should be visible as a real landscape panel near the top of Emotions, not disappear behind a flat grey box.
- The subtle degradé box effect works well in dark modes because it creates depth around `Emotions / Body / Behaviour`. Keep that dark-mode mood, but do not add a flat grey block behind those surfaces in paper or golden modes; those modes should let the paper or golden base breathe.
- The Emotions lane has a third visual test option where the atmospheric image fills the whole Emotions content area below the tab row. In this option the image must break out to the viewport edges, not remain inside the centered app column. The controls remain centered over it with readable contrast. This is an Emotions-only comparison option for now, separate from the larger future idea where Emotions, Missions, and Progress share one full Focus background field.
- The top `/day` tabs remain `Emotions`, `Missions`, and `Progress`. In dark modes, all three titles must stay readable ochre/gold, not dimmed when inactive. In paper and golden modes, selected state should be communicated by the rounded border edge, not by a heavy dark fill behind the label.
- `What are you feeling right now?` and `Emotion Rainbow` should read as paired controls: similar visual format, similar size, and readable text so they feel like two parts of one emotional register.
- Future second design direction for `Emotions / Body / Behaviour`: a circular stacked mode where each surface can be represented by a clear circle with the text below the circle. The labels must be large enough to read on phone; no mini typography.
- The Emotions lane uses one visible atmospheric mood image panel at the top of the lane, rotating slowly between compressed images. Paper and Golden modes use day/sunset imagery; night modes use city/night imagery. The image is not a full-section background and must not sit behind readable controls.
- Collapsible Day panels such as Areas, Tasks, and Daily Rituals keep only their title pill/frame tinted in Paper and Golden modes; opened content stays transparent so the paper or golden app surface remains visible. Dark/night modes may keep darker opened panel fills for contrast.
- When returning from any dark/night design preset to Paper or Golden, the title/header palette falls back to the light Beige family unless the user has explicitly selected another light palette. Dark title palettes must not leak into light modes.
